...
ts1=time.time()
print('starting RH_RP_DB_DH')
if (method=="NSGAII"):
    output_RH_RP_DB_DH = optimize(model_RH_RP_DB_DH, method, iterations, population_size=psize)

if (method=="MOEAD"):
    output_RH_RP_DB_DH = optimize(model_RH_RP_DB_DH, method, population_size=psize)

if (method=="NSGAIII"):
    output_RH_RP_DB_DH = optimize(model_RH_RP_DB_DH, method, divisions_outer=dO)

if (method=="EpsMOEA"):
    output_RH_RP_DB_DH=optimize(model_RH_RP_DB_DH, method, epsilons=epss)

if (method=="BorgMOEA"):
    output_RH_RP_DB_DH=optimize(model_RH_RP_DB_DH, algorithm=method, module="pyborg", population_size=psize, epsilons=epss,NFE=10000)
    
ts2=time.time()
save(output_RH_RP_DB_DH, fnamebase + "_output_RH_RP_DB_DH.json")
print(ts2-ts1)
print(fnamebase)
