...
ts1=time.time()
print('starting RH_RP_WH')
if (method=="NSGAII"):
    output_RH_RP_WH = optimize(model_RH_RP_WH, method, iterations, population_size=psize)

if (method=="MOEAD"):
    output_RH_RP_WH = optimize(model_RH_RP_WH, method, population_size=psize)

if (method=="NSGAIII"):
    output_RH_RP_WH = optimize(model_RH_RP_WH, method, divisions_outer=dO)

if (method=="EpsMOEA"):
    output_RH_RP_WH=optimize(model_RH_RP_WH, method, epsilons=epss)

if (method=="BorgMOEA"):
    output_RH_RP_WH=optimize(model_RH_RP_WH, algorithm=method, module="pyborg", population_size=psize, epsilons=epss,NFE=10000)
    
ts2=time.time()
save(output_RH_RP_WH, fnamebase + "_output_RH_RP_WH.json")
print(ts2-ts1)
print(fnamebase)
