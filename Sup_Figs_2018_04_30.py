...
df00001=df00001.sort_values(netCost)

f=outputPath+a+"2"+b
print(f)
_, F = load(str(f))
df00002=F.as_dataframe()
df00002=df00002.sort_values(netCost)

f=outputPath+a+"10"+b
print(f)
_, F = load(f)
df00010=F.as_dataframe()
df00010=df00010.sort_values(netCost)

f=outputPath+a+"20"+b
print(f)
_, F = load(f)
df00020=F.as_dataframe()
df00020=df00020.sort_values(netCost)

f=outputPath+a+"50"+b
print(f)
_, F = load(f)
df00050=F.as_dataframe()
df00050=df00050.sort_values(netCost)

f=outputPath+a+"100"+b
print(f)
_, F = load(f)
df00100=F.as_dataframe()
df00100=df00100.sort_values(netCost)

f=outputPath+a+"200"+b
print(f)
_, F = load(f)
df00200=F.as_dataframe()
df00200=df00200.sort_values(netCost)

f=outputPath+a+"500"+b
print(f)
_, F = load(f)
df00500=F.as_dataframe()
df00500=df00500.sort_values(netCost)

f=outputPath+a+"1000"+b
print(f)
_, F = load(f)
df01000=F.as_dataframe()
df01000=df01000.sort_values(netCost)

f=outputPath+a+"2000"+b
print(f)
_, F = load(f)
df02000=F.as_dataframe()
df02000=df02000.sort_values(netCost)

f=outputPath+a+"5000"+b
print(f)
_, F = load(f)
df05000=F.as_dataframe()
df05000=df05000.sort_values(netCost)

f=outputPath+a+"10000"+b
print(f)
_, F = load(f)
df10000=F.as_dataframe()
df10000=df10000.sort_values(netCost)

f=outputPath+a+"20000"+b
print(f)
_, F = load(f)
df20000=F.as_dataframe()
df20000=df20000.sort_values(netCost)

df00001[netCost].iloc[0]
A=500
import matplotlib.patches as mpatches
import matplotlib.pyplot as plt
x=[1,2,10,20,50,100,200,500,1000,2000,5000,10000,20000]
y = [df00001[netCost].iloc[0]/A,df00002[netCost].iloc[0]/A,df00010[netCost].iloc[0]/A,df00020[netCost].iloc[0]/A,df00050[netCost].iloc[0]/A,df00100[netCost].iloc[0]/A,df00200[netCost].iloc[0]/A,df00500[netCost].iloc[0]/A,df01000[netCost].iloc[0]/A,df02000[netCost].iloc[0]/A,df05000[netCost].iloc[0]/A,df10000[netCost].iloc[0]/A,df20000[netCost].iloc[0]/A]

figFutures=plt.figure()
fFigure=plt.semilogx(x,y,marker='.')
...
plt.xlabel('States of the World')
plt.ylabel('Minimum net cost ($)')
#plt.xlim(0,investLim/3)
plt.ylim(0,1e11/A*1.5)
...
figFutures.savefig((figPath + method + 'fig_Futures.pdf'),bbox_inches='tight')
plt.show(figFutures)
